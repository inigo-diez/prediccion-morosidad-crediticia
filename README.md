...
Se entrenaron y evaluaron tres modelos de clasificacion supervisada sobre el mismo conjunto de test (sin restriccion de profundidad en Random Forest):
...
### Random Forest — Optimizacion con GridSearchCV

Para afinar el rendimiento del Random Forest se aplico una busqueda exhaustiva de hiperparametros mediante `GridSearchCV` con validacion cruzada de 3 folds. Se exploraron dos configuraciones: una optimizando `accuracy` y otra optimizando `recall`, dado el interes de la entidad en minimizar falsos negativos.
...
**Resultados por combinacion (ordenados por recall, metrica prioritaria):**

| max_depth | n_estimators | Recall medio (CV) |
|-----------|-------------|-------------------|
| **5** | **200** | **0.9667** |
| 5 | 100 | 0.9664 |
| 10 | 200 | 0.9619 |
...
Los mejores hiperparametros segun recall son `max_depth=5` y `n_estimators=200`, con un recall medio en validacion cruzada de **0.967**. Interesantemente, limitar la profundidad del arbol a 5 evita el sobreajuste y generaliza mejor que arboles sin restriccion de profundidad.

Cuando la busqueda se optimiza por `accuracy`, los mejores parametros cambian a `max_depth=20, n_estimators=200` (accuracy media CV = **0.820**), lo que ilustra el compromiso inherente entre precision global y sensibilidad.

**Clasificacion final del modelo optimizado (max_depth=5, n_estimators=200, scoring=recall):**
...
El modelo optimizado por recall captura el **96% de los morosos** reales, aunque a costa de una menor precision en la clase no morosa. Este trade-off es deliberado: en un contexto crediticio, identificar a todos los clientes de riesgo tiene mas valor que evitar falsas alarmas puntuales.
